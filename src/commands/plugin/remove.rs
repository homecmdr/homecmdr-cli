...
use super::add::{adapter_name, canonical_name, short_name, PluginManifest};
...
pub fn run(name: &str) -> Result<()> {
    ...
    if !manifest_path.exists() {
        bail!(
            "plugin '{}' is not installed (no manifest found in {}).",
            short_name(&canonical),
            plugins_dir.display(),
        );
    }

    // Read the installed manifest to determine plugin type and binary name.
    let manifest_str = fs::read_to_string(&manifest_path)
        .with_context(|| format!("failed to read {}", manifest_path.display()))?;
    let manifest: PluginManifest = toml::from_str(&manifest_str)
        .with_context(|| format!("failed to parse {}", manifest_path.display()))?;

    let is_ipc = manifest.plugin.adapter_type == "ipc";

    if is_ipc {
        // Remove the IPC native binary.
        let binary_name = manifest
            .plugin
            .binary
            .as_deref()
            .map(|s| s.to_string())
            .unwrap_or_else(|| format!("{}-adapter", adapter));
        ...
        if binary_path.exists() {
            ...
            println!("  {} not found — skipping.", binary_path.display());
        }
    } else {
    ...
    // Remove .plugin.toml manifest (common to both types).
    ...
    // files from the system plugin directory.
    if std::path::Path::new(crate::commands::config_sync::SYSTEM_CONFIG).exists() {
        ...
        if is_ipc {
            let binary_name = manifest
                ...
                .map(|s| s.to_string())
                .unwrap_or_else(|| format!("{}-adapter", adapter));
            let system_binary = format!("{}/{}", system_plugins_dir, binary_name);
            let _ = crate::commands::config_sync::sudo_run(&[
                "rm", "-f", &system_binary, &system_toml,
            ]);
        } else {
            let system_wasm = format!("{}/{}.wasm", system_plugins_dir, adapter);
            let _ = crate::commands::config_sync::sudo_run(&[
                "rm", "-f", &system_wasm, &system_toml,
            ]);
        }
    }
